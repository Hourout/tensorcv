...
def MNIST_Kuzushiji_10(root):
    start = time.time()
    assert tf.gfile.IsDirectory(root), '`root` should be directory.'
    task_path = os.path.join(root, 'MNIST_Kuzushiji_10')
    ...
    print('MNIST_Kuzushiji_10 dataset download completed, run time %d min %.2f sec' %divmod((time.time()-start), 60))
    return task_path
